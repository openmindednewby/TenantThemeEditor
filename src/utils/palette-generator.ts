...
export function hslToHex(h: number, s: number, l: number): string {
  ...
  const rVal = Math.round(hueToRgb(p, q, hNorm + ONE_THIRD) * RGB_MAX);
  const gVal = Math.round(hueToRgb(p, q, hNorm) * RGB_MAX);
  const bVal = Math.round(hueToRgb(p, q, hNorm - ONE_THIRD) * RGB_MAX);
